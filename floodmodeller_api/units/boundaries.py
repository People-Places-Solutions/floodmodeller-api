...
class FEHBDY(Unit):
    ...
    Args:
        ...
        calcrates(str): Override of snowmelt rates provided with calculated values. Override on (CALCRATES = 'CALCTRUE'), Override off (CALCRATES = 'CALCFALSE'), blanck (== CALCFALSE)
    ...
    def _read(self, fehbdy_block):
        ...
        #  line 3 
        feh_params = split_10_char(f"{fehbdy_block[2]:<30}")
        self._z = feh_params[0] # parameter referenced in Flood Modeller help but not exposed in UI.  Field looks to be inconsistently used across ieds
        ...
        #  Line 7
        feh_params1 = split_10_char(f"{fehbdy_block[6]:<80}")
        self.storm_area = _to_float(feh_params1[0], 0.000)
        self.storm_duration = _to_float(feh_params1[1], 0.000) # TODO:Storm duration (hrs). Should be the nearest odd integer multiple of timestep - add validation
        self.snow_melt_rate = _to_float(feh_params1[3], 0.000)
        self.s100 = _to_float(feh_params1[4], 0.000)
        self.temperature = _to_float(feh_params1[5], 0.000)
        self.antecedent_melt_rate = _to_float(feh_params1[6], 0.000)
        self.calcrates = _to_str(feh_params1[7], "CALCFALSE")
        ...
        if self.erflag == "FSRER":
            self.erflag = "FEHER" # update the legacy default paramter of "FSRBDY" to FEHBDY.  FEHBDY is shown in UI when FSRBDY in ied file

        #  Line 10
        if self.erflag == "FEHER":
            feh_params2 = split_10_char(f"{fehbdy_block[9]:<90}")
            ...
            self.ddf_c = _to_float(feh_params2[3], 0.000)
            self.ddf_d1 = _to_float(feh_params2[4], 0.000)
            self.ddf_d2 = _to_float(feh_params2[5], 0.000)
            self.ddf_d3 = _to_float(feh_params2[6], 0.000)
            self.ddf_e = _to_float(feh_params2[7], 0.000)
            self.ddf_f = _to_float(feh_params2[8], 0.000)
