...
def test_entry_points(tool):
    tool.generate_gui()
    [param.name for param in tool.parameters] == [name for name in tool.root_entries.keys()]
...
def test_check_parameters():
    ...
    class MyTool(FMTool):
        parameters = [
            Parameter(name="param1", dtype=str, description="Description 1", help_text="Help text 1"),
            Parameter(name="param1", dtype=str, description="Description 2", help_text="Help text 2")
        ]
...
def test_run():
    # Test that the run method calls the entry_point method with the correct arguments
    class MyTool(FMTool):
        @classmethod
        def entry_point(cls, param1, param2):
...
def test_run_from_command_line():
    ...
    with patch("sys.argv", ["my_script.py", "--param1", "value1", "--param2", "value2"]):
...
def test_generate_gui():
    ...
    class MyTool(FMTool):
        parameters = [
            Parameter(name="param1", dtype=str, description="Description 1", help_text="Help text 1"),
            Parameter(name="param2", dtype=int, description="Description 2", help_text="Help text 2")
        ]
...
def test_run_gui_callback():
    ...
    # Call the callback function and check that the entry_point method is called with the correct arguments
