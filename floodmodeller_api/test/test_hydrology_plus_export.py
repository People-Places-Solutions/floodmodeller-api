...
def test_data_metadata(expected_metada: dict[str, str], hydrology_plus_export_object: HydrologyPlusExport ):
    """Compares the metada between the csv and the class"""
    assert expected_metada == hydrology_plus_export_object.metadata


def test_data_flows_df(expected_dataframe: pd.DataFrame, hydrology_plus_export_object: HydrologyPlusExport ):
