...
MUSK_UNIT_NAMES = [unit_name for unit_name, _ in MUSK_UNIT_EXPECTED_CASES]


@pytest.mark.parametrize(("unit_name", "expected"), MUSK_UNIT_EXPECTED_CASES, ids=MUSK_UNIT_NAMES)
def test_musk_read_from_dat(test_workspace, unit_name, expected):
    river_section = DAT(test_workspace / "All Units 4_6.DAT").sections[unit_name]

    for attr, expected_value in expected.items():
        if isinstance(expected_value, pd.DataFrame):
            actual_value = getattr(river_section, attr)
            pd.testing.assert_frame_equal(actual_value, expected_value, check_index_type=False)
        else:
            assert getattr(river_section, attr) == expected_value


@pytest.mark.parametrize("unit_name", MUSK_UNIT_NAMES)
def test_musk_write_from_dat(test_workspace, unit_name):
...
def test_active_data_is_data_for_non_section_subtypes(test_workspace):
    unit = DAT(test_workspace / "All Units 4_6.DAT").sections["UNIT042"]

    pd.testing.assert_frame_equal(unit.active_data, unit.data)
