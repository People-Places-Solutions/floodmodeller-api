...
def to_json(obj):
    ...
    # function to check if it is serizeable or not.
    def is_jsonable(x):
        try:
            json.dumps(x)
            return True
    ...
    dictObj = {}
    cla = obj.__class__
    dictObj["API Class"] = str(cla)[:]   # add slicing
    dictObj["API Version"] = VERSION_API  # __version__
    ...
    with open(r"C:\PROJECTS\FLOOD_MODELLER_API\JSON\floodmodeller-api\demo\data\dat_03.json", "w") as json_file:
