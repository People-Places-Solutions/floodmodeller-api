...
xs_attributes["intersect"] = [
    next(
        (row["ID"] for _, row in nwk_attributes.iterrows() if row["start"].intersects(xs_geometry)),
        None,
    )
    for xs_geometry in xs_attributes["geometry"]
]
xs_attributes["mid_intersect"] = [
    next(
        (
            river_row["ID"]
            for river_index, river_row in nwk_attributes.iterrows()
...
xs_attributes["end_intersect"] = [
    next(
        (row["ID"] for _, row in nwk_attributes.iterrows() if row["end"].intersects(xs_geometry)),
        None,
    )
    for xs_geometry in xs_attributes["geometry"]
]
...
for i, row in nwk_attributes.iterrows():
    end_point = row["end"]
    intersected_rows = nwk_attributes[
        ~nwk_attributes.index.isin([i]) & nwk_attributes.geometry.intersects(end_point)
    ]
...
for i, row in nwk_attributes.iterrows():
    ...
    intersected_rows = nwk_attributes[
        ~nwk_attributes.index.isin([i]) & nwk_attributes.geometry.intersects(start_point)
    ]
...
full_flag_dict = nwk_attributes.set_index("ID")[["Flag", "n_nF_Cd", "length", "start"]].to_dict(
    orient="index"
)
...
for i, row in xs_attributes.iterrows():
    ...
    if "start" in row["Flag"] or "join_start" in row["Flag"]:
        ...
        next_row_index = xs_attributes[
            (xs_attributes["end_intersect"] == intersect_value) & (xs_attributes["Flag"] == "")
        ].index
...
cross_sections["Name"] = [
    "RIV" + str(i).zfill(3) for i in range(1, len(cross_sections) + 1)
]  ###CHECK this
